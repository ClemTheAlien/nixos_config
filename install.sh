# --- Configuration ---
SOURCE_NAME="nixos_config"
DEST_PATH="$HOME/.nixos_config"
...
"
sleep 2
# 1. Safety Check: Does the source even exist?
if [ ! -d "$SOURCE_NAME" ]; then
    echo "❌ Error: Source folder '$SOURCE_NAME' not found in current directory."
    exit 1
fi

# 2. Clean Up: Delete existing destination if it exists
if [ -d "$DEST_PATH" ]; then
    echo "🗑️  Existing config found at $DEST_PATH. Deleting for a clean install..."
    rm -rf "$DEST_PATH"
fi

# 3. Create the destination directory fresh
echo "Creating fresh directory at $DEST_PATH..."
mkdir -p "$DEST_PATH"

# 4. Copy contents
echo "🚚 Installing files from $SOURCE_NAME to $DEST_PATH..."
cp -av "$SOURCE_NAME/." "$DEST_PATH"

if [ $? -eq 0 ]; then
    echo "✅ Installation complete! Your clean config is ready at $DEST_PATH"
else

...
* Read the guide above and keep the following in mind. 
  ...
  * **C)** You might have to remove and copy the new ```hardware-configuration.nix``` file to the ```hosts``` folder depending on which profile you are using.
  * **D)** It is your choice but updating the system is streamlined through the moving of the dotfiles into a hidden folder that the builtin updating script utilizes. After install it is imperative that you do not use the git cloned repo in /etc/nixos but delete it and re-git-clone it in the home folder where you will run the ```install.sh```. Then and only then can you use the builtin ```world_update``` and ```boot_update``` If you desire to make any changes edit the ```nixos_config``` folder and re-run the ```install.sh``` script.
